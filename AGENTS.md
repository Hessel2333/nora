...
- 订单、配方版本、审核生产需求、快照物料展开、生产批次确认、基于冻结 v2 配方的一批次一工单释放、工单现场状态控制，以及库存库位/批次/追加式流水/余额投影、期初入账和工单领退料是后端 MVP。
- Dashboard、旧排程详情、MES 执行、数字孪生、预测、门户和 Showroom 是明确 Demo，不是生产能力；生产准备、生产工单状态与领退料控制、库存台账已有真实后端路径。
- 配方版本内有序工艺步骤、工序级阶段投料、工单领退料，以及单批次报产/质量判定/合格成品入库已有首个 MVP；并行/返工路线、实际工序投料、完整单位换算、采购收货、真实 MES、完整质检追溯和实际成本尚未实现。
