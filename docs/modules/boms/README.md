...
BOM 模块管理产出商品的配方版本、净配工艺路线、阶段投料和配方级用量试算。订单中多个商品的合并物料需求属于订单与生产需求用例；BOM 模块不负责库存可用量或采购建议。
...
└── BomVersion（V1.0 / V1.1 / V2.0）
    ├── BomOperation（有序净配步骤）
    └── BomItem（直接子物料、投入步骤、净用量、出成率、单位成本快照）
```
...
配方页的用量试算只放大当前配方版本及其递归半成品，不接受订单参数。多个订单行的合并展开由订单用例处理，已审核订单必须读取冻结快照。
