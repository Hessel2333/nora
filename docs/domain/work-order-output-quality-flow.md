...
- 明确不做：工序级投料、库存报废处置、返工路线、让步接收和部分批次累计完工；工单级耗用/报损必须在本流程前完成核销。
...
| --- | --- | --- | --- | --- | --- | --- |
| `ReportOutput` | `running` | revision 一致、物料已领齐并完整核销、无待检/放行产出、批次号唯一、单位一致、差异有原因 | `awaiting_quality` | 创建产出和待检批次；同步生产批次 | `output_reported` | 现场操作员 |
...
| --- | --- | --- | --- | --- |
| 工单/批次状态、revision 与物料核销 | 是 | 接受命令 | `409` 阻断 | 生产主管 |
