...
返回工单冻结配方的叶子原料需求、累计领料、累计退料、净领用、实际耗用、报损、待核销、剩余计划量、已领批次去向和最近核销事实，以及当前已放行批次余额（按近效期优先）。正式需求不得读取当前 BOM。
...
请求体与领料相同。退料回到同一余额行，累计数量不能超过该工单对同一库位、产品和批次的待核销量，即领料减既有退料、实际耗用与报损。
...
| 409 | 批次号已存在、库存不足、超计划领料、退料包含已核销数量、revision/工单状态冲突 | 刷新后更正数量或状态 |
...
实际耗用和现场报损是已领物料的执行去向，不是第二次库存出库。它们由 `POST /api/v1/work-orders/:id/material-usages` 追加 `WorkOrderMaterialUsage`，不会新增库存流水或改变余额；决策依据见 [ADR-0008](../decisions/0008-issued-material-disposition-is-not-a-second-stock-movement.md)。
