...
请求头：`Idempotency-Key: work-order-material-usage:<uuid>`。
...
- `disposition` 为 `consumed` 或 `scrapped`；`scrapped` 必须填写原因。
- 只允许生产中工单登记。所选余额行只用于解析原领料的产品、库存批次和库位；核销不会修改余额，也不会新增库存流水。
- 累计耗用和报损不能超过同一工单、产品、库位和批次的净领用量；命令与退料共享工单锁。
- 成功返回新核销事实及最新工单物料投影。重复幂等键返回同一事实。
...
仅允许 `running → awaiting_quality`。每类冻结叶子原料必须净领齐计划数量，且已领批次全部归入实际耗用、报损或退料；否则返回 `409`。守卫通过后系统创建 `WorkOrderOutput.pending_quality` 和 `InventoryLot.pending`，复制冻结配方末道工序温度上下限，并同步生产批次；不创建库存流水。实际数量与计划不一致时必须填写差异原因。
...
| 409 | 状态非法、revision 过期、超已领核销、物料未领齐/未核销、批次号冲突、已有待检/放行产出、幂等键被其他命令使用或批次投影不一致 | 刷新后重新判断 |
...
- 物料核销只追加且不重复改变库存；报产前每类冻结叶子原料满足 `计划 = 净领用 = 实际耗用 + 报损`。
