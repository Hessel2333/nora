...
生产计划的合批/拆批、批次确认和一批次一工单释放已实现，当前关系为：

```text
生产需求行 ← 需求分配（allocated_quantity）→ 生产批次 → 生产工单
```
...
| 下达工单 | `ProductionBatch.confirmed` | revision 未变化；冻结 v2 配方和工作中心完整 | 创建工单和事件；批次进入 `released` | 生产主管 |
