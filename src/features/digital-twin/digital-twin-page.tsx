...
export function DigitalTwinPage() {
  ...
  return <>
    <PageHeader title="工厂态势 · 呆大厨工厂" metadata={<HelpTip title="工厂态势说明">空间、任务、环境与视频状态均为体验数据。</HelpTip>} actions={<><Button variant="secondary" size="sm" onClick={enterFullscreen}><Maximize2 size={15} />全屏</Button><Button variant="secondary" size="sm" onClick={() => setLastRefresh(new Date().toLocaleTimeString("zh-CN", { hour12: false }))}><RefreshCw size={15} />刷新 {lastRefresh}</Button></>} />
      ...
      <Card ref={mapRef} className="order-1 min-w-0 overflow-hidden bg-white 2xl:order-2">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e8edf3] px-4 py-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-[10px] text-[#7b879b]">17 个分区 · 15 个业务区域</span>
        ...
        <div className="border-t border-[#e8edf3] p-3">
          {activeView === "environment" ? <>
            ...
            <div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-xs font-semibold text-[#34425e]">蔬菜批次流转路径</p><p className="mt-0.5 text-[10px] text-[#667085]">DEMO-BATCH-018 · 从收货到发货</p></div><Badge tone="info">流程联动</Badge></div>
