...
import { Camera, CircleDot, Download, Expand, HardDrive, RefreshCw, ShieldCheck, Video, Wifi, X } from "lucide-react";
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex max-h-[calc(100vh-24px)] w-[min(1240px,calc(100vw-24px))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-[#dce3ed] bg-white shadow-[0_28px_90px_rgba(7,17,32,.32)] focus:outline-none">
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[#e6eaf1] px-4 py-4 sm:px-6">
          <div className="min-w-0">
            <div className="mb-1 flex flex-wrap items-center gap-2"><span className="text-[10px] font-bold tracking-[0.09em] text-[#7d899e]">{room.id}</span><Badge tone="success"><CircleDot size={10} />摄像头在线</Badge>{isDemo ? <Badge tone="warning">演示画面</Badge> : <Badge tone="info">实时画面</Badge>}</div>
        ...
        <div className="nora-scrollbar min-h-0 flex-1 overflow-y-auto bg-[#f6f8fb] p-3 sm:p-4">
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_286px]">
            <main className="min-w-0 overflow-hidden rounded-xl border border-[#172234] bg-[#07101d] shadow-sm">
              <div ref={playerRef} className="relative aspect-video overflow-hidden bg-[#07101d]">
                ...
                <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between gap-3 p-3 text-[11px] text-white sm:p-4">
                  <span className="inline-flex items-center gap-1.5 rounded-md bg-[#07101d]/78 px-2 py-1 font-semibold backdrop-blur-sm"><span className={cn("h-2 w-2 rounded-full", isDemo ? "bg-[#f59e0b]" : "bg-[#ef4444]")} />{isDemo ? "演示画面" : "实时"}</span>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/10 bg-[#0b1524] px-3 py-3 sm:px-4">
                <div className="flex items-center gap-2 text-[11px] text-white/68"><CircleDot size={13} className="text-[#34d399]" />{isDemo ? "演示视频源已连接" : "实时视频流已连接"}</div>
            ...
            <aside className="space-y-3">
              <section className="rounded-xl border border-[#e1e6ee] bg-white p-4">
                <div className="flex items-center justify-between gap-3"><h3 className="text-sm font-semibold text-[#253451]">摄像头机位</h3><Badge tone="success">{cameras.length} 路在线</Badge></div>
              ...
              <section className="rounded-xl border border-[#e1e6ee] bg-white p-4">
                ...
                <div className="mt-3 space-y-3">
                  <div className="flex items-center gap-3"><IconBox icon={Wifi} tone="success" size="sm" /><div><span className="block text-[10px] text-[#8a95a7]">连接状态</span><strong className="text-xs text-[#34425e]">稳定 · 38 ms</strong></div></div>
                  <div className="flex items-center gap-3"><IconBox icon={Video} tone="info" size="sm" /><div><span className="block text-[10px] text-[#8a95a7]">录像状态</span><strong className="text-xs text-[#34425e]">{activeCamera.recording ? "连续录像中" : "未录像"}</strong></div></div>
                  <div className="flex items-center gap-3"><IconBox icon={HardDrive} tone="neutral" size="sm" /><div><span className="block text-[10px] text-[#8a95a7]">录像保留</span><strong className="text-xs text-[#34425e]">最近 {activeCamera.retentionDays} 天</strong></div></div>
                </div>
              </section>

              <section className="rounded-xl border border-[#f0d291] bg-[#fff8e8] p-4">
                <div className="flex gap-3"><ShieldCheck size={18} className="mt-0.5 shrink-0 text-[#cf7b00]" /><div><h3 className="text-xs font-semibold text-[#8f5a08]">当前为演示画面</h3><p className="mt-1 text-[11px] leading-5 text-[#947140]">接入工厂摄像头视频流后，此处将自动播放对应房间的实时画面。</p></div></div>
              </section>
            </aside>
          </div>
        </div>
