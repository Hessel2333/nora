...
import { AlertTriangle, ArrowRight, Boxes, BrainCircuit, CheckCircle2, ClipboardList, Factory, PackageSearch, Play, RotateCw, ShoppingCart, Sparkles, Truck, UsersRound } from "lucide-react";
...
export function DashboardPage() {
  ...
  return <>
    <PageHeader title="早上好，张伟" description="以下是美味中央厨房今日的运营概览" actions={<><Button variant="secondary" size="sm" onClick={() => setGuideOpen(true)}>新手引导</Button><Button size="sm" onClick={() => setJourneyOpen(true)}><Play size={15} />演示业务流程</Button></>} />
    ...
    <Card className="mt-3 overflow-hidden">
      <SectionTitle title="业务运行地图" description="从客户需求到现场执行，再到经营反馈" action={<button type="button" onClick={() => setJourneyOpen(true)} className="text-xs font-medium text-[#1768f2]">播放完整旅程</button>} />
      <div className="nora-scrollbar overflow-x-auto p-4">
        <div className="flex min-w-[980px] items-stretch gap-2">
          {flow.map((item, index) => <div key={item.title} className="flex flex-1 items-center gap-2"><Link href={item.href} className={`focus-ring group flex min-h-[112px] flex-1 flex-col justify-between rounded-xl border p-3.5 transition ${flowStep === index ? "border-[#1768f2] bg-[#edf4ff] shadow-[0_0_0_3px_rgba(23,104,242,.08)]" : "border-[#e5eaf1] bg-white hover:border-[#c9d8ef] hover:bg-[#fafcff]"}`}><div className="flex items-start justify-between"><IconBox icon={item.icon} tone={index < 2 ? "info" : index < 4 ? "purple" : index === 4 ? "success" : "warning"} size="sm" /><span className="text-[10px] font-semibold text-[#9aa4b6]">{item.layer}</span></div><div><h3 className="text-sm font-semibold text-[#22304d]">{item.title}</h3><p className="mt-1 text-xs text-[#78849a]">{item.detail}</p></div></Link>{index < flow.length - 1 && <ArrowRight size={17} className="shrink-0 text-[#a5afbf]" />}</div>)}
        </div>
      </div>
    </Card>

    <div className="mt-3 grid gap-3 xl:grid-cols-[0.9fr_1.4fr_1.2fr]">
      <Card><SectionTitle title="生产完成率" /><div className="flex items-center justify-center gap-6 p-5"><div className="relative flex h-36 w-36 items-center justify-center rounded-full" style={{ background: `conic-gradient(#1768f2 ${completion * 3.6}deg, #e8edf4 0deg)` }}><div className="flex h-[116px] w-[116px] flex-col items-center justify-center rounded-full bg-white"><strong className="text-4xl font-medium tracking-[-0.06em]">{completion}<span className="text-lg">%</span></strong><span className="mt-1 text-xs text-[#7d899e]">今日完成率</span></div></div><div className="min-w-[120px] space-y-3 text-xs"><div><span className="text-[#8792a5]">计划产量</span><strong className="mt-1 block text-[#253451]">12,560 份</strong></div><div><span className="text-[#8792a5]">已完成</span><strong className="mt-1 block text-[#253451]">9,817 份</strong></div><div className="text-[#079b71]">较昨日 ↑ 8.6%</div></div></div></Card>
    ...
    <ProcessJourney open={journeyOpen} onOpenChange={setJourneyOpen} onStepChange={setFlowStep} />
  </>;
}
