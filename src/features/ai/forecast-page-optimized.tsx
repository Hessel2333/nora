...
export function ForecastPage() {
  ...
  const chartDates = useMemo(() => Array.from({ length: 10 }, (_, index) => {
    const date = new Date();
  ...
  const option = useMemo(() => ({
    ...
    legend: { data: ["历史销量", "预测销量"], right: 12, top: 8, textStyle: { color: "#66738a" } },
    xAxis: {
      type: "category",
      data: chartDates,
    ...
    series: [
      ...
      { name: "预测销量", type: "line", smooth: true, data: [null, null, null, null, null, 2520, 2460, 2680, 2840, 2730], lineStyle: { color: "#1768f2", width: 3, type: "dashed" }, itemStyle: { color: "#1768f2" }, areaStyle: { color: "rgba(23,104,242,.08)" } },
    ],
  }), [chartDates]);
  ...
  const generate = async () => {
    ...
    await add({
      id: `ai-${Date.now()}`,
      code: `DEMO-SO-${String(Date.now()).slice(-6)}`,
      customerId: customer.id,
      customerName: customer.name,
      deliveryAt: `${tomorrow} 11:00`,
      status: "draft",
      source: "AI预测",
  ...
  return <>
    <PageHeader
      title="销售预测"
      metadata={<HelpTip title="预测说明">当前结果用于体验预测与订单草稿流程，不用于正式业务决策。</HelpTip>}
      actions={<Button variant="secondary" onClick={() => setReset(true)}><RefreshCw size={16} />重置</Button>}
    />

    {reset && <div role="status" className="mb-4 flex items-center justify-between gap-3 rounded-[14px] border border-[#b9e5d7] bg-[#eefaf6] px-4 py-3 text-sm text-[#08775b]">
      <span className="flex items-center gap-2 font-medium"><CheckCircle2 size={17} />预测结果已恢复</span>
      <button type="button" onClick={() => setReset(false)} className="focus-ring shrink-0 rounded-md px-2 py-1 text-xs font-semibold hover:bg-white/70">关闭</button>
    </div>}
    ...
    <div className="horizontal-snap -mx-4 mb-4 grid grid-flow-col auto-cols-[82%] gap-3 overflow-x-auto px-4 pb-1 sm:mx-0 sm:grid-flow-row sm:auto-cols-auto sm:grid-cols-3 sm:px-0">
      <MetricCard label="预测销量" value="8,420" suffix="份" icon={TrendingUp} change="较周均值 +12.6%" />
      <MetricCard label="回测准确率" value="91.8" suffix="%" icon={Sparkles} tone="purple" />
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
      <Card className="overflow-hidden">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#e8edf3] p-4">
          <div><h2 className="font-semibold">宫保鸡丁净菜包销量趋势</h2><p className="mt-0.5 text-xs text-[#8792a6]">近 7 天实际销量与未来 3 天预测</p></div>
          <Badge tone="info">参考区间 ±8.2%</Badge>
        </div>
      ...
      <Card className="p-5">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1"><h2 className="font-semibold">生成订单草稿</h2><HelpTip title="订单草稿">预测结果只会生成草稿，确认后再提交审核。</HelpTip></div>
        </div>
        <div className="mt-5 space-y-4">
          <Field label="预测客户"><select className={inputClass}><option>盒马鲜生南山店</option><option>华润万家深圳福田店</option></select></Field>
          <Field label="交付日期"><input type="date" defaultValue={tomorrow} className={inputClass} /></Field>
        ...
        {generated ? <div role="status" className="mt-5 rounded-xl bg-[#e9f8f3] p-4">
          ...
          <p className="mt-1 text-xs text-[#5d7f75]">宫保鸡丁净菜包 980份 · 可在订单中心继续编辑。</p>
        ...
        </div> : <Button onClick={() => void generate()} className="mt-5 w-full"><Sparkles size={16} />生成订单草稿</Button>}
      </Card>
    </div>
    ...
    <Card className="mt-4 p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        {[
          ...
          [TrendingUp, "近期趋势", "宫保鸡丁净菜包连续 4 日增长"],
