...
const exceptions: ExceptionItem[] = [
  { id: "chicken", rank: 1, name: "冷冻鸡胸肉", code: "RM01234", shortage: 84.5, unit: "kg", category: "shortage", demand: 204.5, available: 120, coverage: 67, amount: 1749.15, formula: "180kg ÷ 88% = 204.5kg", path: "订单 → 宫保鸡丁净菜包 → 腌制鸡肉丁 → 冷冻鸡胸肉" },
  { id: "bamboo", rank: 2, name: "鲜冬笋丝", code: "RM02116", shortage: 14, unit: "kg", category: "shortage", demand: 64, available: 50, coverage: 78, amount: 294, formula: "57.6kg ÷ 90% = 64kg", path: "订单 → 鱼香肉丝净菜包 → 净配时蔬 → 鲜冬笋丝" },
  { id: "peanut", rank: 3, name: "花生米", code: "RM03108", shortage: 12, unit: "kg", category: "shortage", demand: 48, available: 36, coverage: 75, amount: 180, formula: "47kg ÷ 98% = 48kg", path: "订单 → 宫保鸡丁净菜包 → 宫保辅料包 → 花生米" },
  { id: "garlic", rank: 4, name: "葱姜蒜净料", code: "RM03004", shortage: 8, unit: "kg", category: "shared", demand: 28, available: 20, coverage: 71, amount: 68, formula: "26.9kg ÷ 96% = 28kg", path: "订单 → 2 个净菜产品 → 葱姜蒜净料" },
];

const rows = [
  { id: "chicken", name: "冷冻鸡胸肉", code: "RM01234", category: "禽肉类", demand: "204.5", onHand: "360", allocated: "240", available: "120", coverage: 67, shortage: "84.5", formula: "180 ÷ 88%", path: "宫保鸡丁净菜包" },
  { id: "chicken-source", name: "来自 宫保鸡丁净菜包", code: "1,200 份 × 标准净用量", category: "来源拆分", demand: "204.5", onHand: "—", allocated: "—", available: "—", coverage: 67, shortage: "84.5", formula: "180 ÷ 88%", path: "完整路径" },
  { id: "bamboo", name: "鲜冬笋丝", code: "RM02116", category: "蔬菜类", demand: "64.0", onHand: "68", allocated: "18", available: "50", coverage: 78, shortage: "14", formula: "57.6 ÷ 90%", path: "鱼香肉丝净菜包" },
  { id: "peanut", name: "花生米", code: "RM03108", category: "干货类", demand: "48.0", onHand: "46", allocated: "10", available: "36", coverage: 75, shortage: "12", formula: "47 ÷ 98%", path: "宫保鸡丁净菜包" },
  { id: "garlic", name: "葱姜蒜净料", code: "RM03004", category: "净配调料", demand: "28.0", onHand: "42", allocated: "22", available: "20", coverage: 71, shortage: "8", formula: "26.9 ÷ 96%", path: "2 个净菜产品" },
  { id: "fungus", name: "水发木耳", code: "RM02128", category: "菌菇类", demand: "72.0", onHand: "108", allocated: "18", available: "90", coverage: 100, shortage: "0", formula: "64.8 ÷ 90%", path: "鱼香肉丝净菜包" },
];

const dishRows = [
  { id: "dish-kungpao", name: "宫保鸡丁净菜包", code: "CP0001 · 1,200 份", category: "产品来源", demand: "—", onHand: "—", allocated: "—", available: "—", coverage: 100, shortage: "0", formula: "BOM V2.1", path: "3 种原料" },
  { ...rows[0], category: "宫保鸡丁净菜包", path: "完整路径" },
  { ...rows[3], category: "宫保鸡丁净菜包", path: "共享原料" },
  { ...rows[4], id: "garlic-kungpao", category: "宫保鸡丁净菜包", demand: "16.0", shortage: "4.6", path: "共享原料" },
  { id: "dish-yuxiang", name: "鱼香肉丝净菜包", code: "CP0002 · 800 份", category: "产品来源", demand: "—", onHand: "—", allocated: "—", available: "—", coverage: 100, shortage: "0", formula: "BOM V1.8", path: "3 种原料" },
  { ...rows[2], category: "鱼香肉丝净菜包", path: "完整路径" },
  { ...rows[5], category: "鱼香肉丝净菜包", path: "完整路径" },
  { ...rows[4], id: "garlic-yuxiang", category: "鱼香肉丝净菜包", demand: "12.0", shortage: "3.4", path: "共享原料" },
];
...
export function MrpWorkbenchConcept() {
  ...
  const visibleRows = useMemo(() => {
    ...
    return (group === "dish" ? dishRows : rows).filter((row) => {
      if (shortageOnly && Number.parseFloat(row.shortage) <= 0) return false;
      return !normalized || `${row.name}${row.code}${row.category}${row.path}`.toLowerCase().includes(normalized);
    });
  }, [group, search, shortageOnly]);
  const suggestionAdded = addedSuggestionIds.includes(selected.id);
  ...
  return <div className="-m-4 flex h-[calc(100vh-64px)] min-h-[760px] flex-col overflow-hidden bg-[#f5f7fb] lg:-m-6">
    <header className="flex h-[76px] shrink-0 items-center justify-between border-b border-[#dfe5ed] bg-white px-6">
      <div><div className="flex items-center gap-2"><h1 className="text-[24px] font-semibold tracking-[-0.035em] text-[#111c3b]">物料需求分析</h1><HelpTip title="分析说明">当前数量、库存和建议用于体验分析流程，不会生成正式采购或生产记录。</HelpTip></div></div>
      <div className="flex items-end gap-2">
        <div><span className="mb-1 block text-[9px] font-semibold text-[#7d899e]">销售订单</span><span className="flex h-9 w-[300px] items-center rounded-[9px] border border-[#dce3ed] bg-[#f8fafc] px-3 text-xs text-[#263451]">DEMO-SO-0018 · 华润万家深圳福田店</span></div>
        <div><span className="mb-1 block text-[9px] font-semibold text-[#7d899e]">需求日</span><span className="flex h-9 items-center rounded-[9px] border border-[#dce3ed] bg-[#f8fafc] px-3 text-xs text-[#263451]">基准日</span></div>
        <Button size="sm" className="h-9" onClick={() => setSuggestionGenerated(true)}><PackageOpen size={14} />{suggestionGenerated ? "建议已生成" : "生成建议"}</Button>
      </div>
    </header>
    ...
    <main className="grid min-h-0 flex-1 grid-rows-[minmax(330px,.9fr)_16px_minmax(280px,1.1fr)] p-3">
      <section className="grid min-h-0 grid-cols-[238px_minmax(520px,1fr)_286px] overflow-hidden rounded-xl border border-[#dfe5ed] bg-white shadow-[0_2px_10px_rgba(36,52,82,.04)]">
        ...
        <div className="relative min-w-0 overflow-hidden bg-[#fbfcfe]">
          <div className="absolute left-3 top-3 z-10 flex h-8 items-center gap-1 rounded-lg border border-[#dce3ed] bg-white p-1 shadow-sm"><button type="button" aria-label="缩小流程图" onClick={() => setZoom((value) => Math.max(70, value - 10))} className="rounded p-1 text-[#69768d] hover:bg-[#f1f4f8]"><ZoomOut size={13} /></button><span className="w-8 text-center text-[10px]">{zoom}%</span><button type="button" aria-label="放大流程图" onClick={() => setZoom((value) => Math.min(120, value + 10))} className="rounded p-1 text-[#69768d] hover:bg-[#f1f4f8]"><ZoomIn size={13} /></button></div>
          ...
          {view === "graph" ? <div className="h-full transition-transform" style={{ transform: `scale(${zoom / 100})` }}><FlowCanvas selected={selected} onSelect={setSelectedId} /></div> : <div className="nora-scrollbar h-full overflow-auto p-14"><div className="space-y-2">{filteredExceptions.map((item) => <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={cn("focus-ring flex w-full items-center justify-between rounded-lg border px-3 py-2 text-left", item.id === selected.id ? "border-[#83adf7] bg-[#edf4ff]" : "border-[#e1e6ee] bg-white hover:bg-[#f8fafc]")}><span><strong className="block text-xs text-[#273553]">{item.name}</strong><span className="text-[10px] text-[#7b879d]">{item.path}</span></span><Badge tone="danger">缺 {item.shortage} {item.unit}</Badge></button>)}</div></div>}
        </div>
        ...
        <aside className="border-l border-[#e5e9ef] p-3">
          ...
          <Button size="sm" className="mt-1.5 w-full" onClick={() => setAddedSuggestionIds((ids) => ids.includes(selected.id) ? ids : [...ids, selected.id])} disabled={suggestionAdded}><ShoppingCart size={14} />{suggestionAdded ? "已加入采购建议" : "加入采购建议"}</Button>
        </aside>
      </section>
      ...
      <section className="min-h-0 overflow-hidden rounded-xl border border-[#dfe5ed] bg-white shadow-[0_2px_10px_rgba(36,52,82,.04)]">
        <div className="flex h-12 items-center gap-2 border-b border-[#e6eaf0] px-3">
          <div className="flex h-8 rounded-lg border border-[#dce3ed] bg-[#f6f8fb] p-0.5"><button onClick={() => setGroup("material")} className={cn("rounded-md px-3 text-[10px] font-medium", group === "material" ? "bg-white text-[#1768f2] shadow-sm" : "text-[#68758d]")}>按原料</button><button onClick={() => setGroup("dish")} className={cn("rounded-md px-3 text-[10px] font-medium", group === "dish" ? "bg-white text-[#1768f2] shadow-sm" : "text-[#68758d]")}>按产品来源</button></div>
          <div className="relative w-[300px]"><Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#8f9bad]" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="h-8 w-full rounded-lg border border-[#dce3ed] pl-8 pr-3 text-[10px] outline-none" placeholder="搜索原料、编码或来源产品" /></div>
          <div className="ml-auto flex gap-1"><button type="button" onClick={() => setShortageOnly((value) => !value)} aria-pressed={shortageOnly} className={cn("flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-[10px]", shortageOnly ? "border-[#83adf7] bg-[#edf4ff] text-[#1768f2]" : "border-[#dce3ed] text-[#53617a]")}><AlertTriangle size={12} />仅看缺料</button></div>
        </div>
...
function FlowCanvas({ selected, onSelect }: { selected: ExceptionItem; onSelect: (id: string) => void }) {
  return <div className="relative h-full min-h-[330px] overflow-hidden bg-[radial-gradient(circle_at_1px_1px,#dfe5ee_1px,transparent_0)] bg-[size:18px_18px] pt-12">
    ...
    <GraphNode className="left-[4%] top-[42%]" title="订单需求" code="DEMO-SC-018" value="2,000 份" />
    <GraphNode className="left-[27%] top-[26%]" title="宫保鸡丁净菜包" code="CP0001" value="1,200 份" tone="purple" />
    <GraphNode className="left-[27%] top-[69%] opacity-35" title="鱼香肉丝净菜包" code="CP0002" value="800 份" tone="purple" />
